...
The Personal Study Portal will be implemented as a Next.js app at the repository root, with application code under `src/`, while Study Content will live in a dedicated Content Library at `content/modules/`. This keeps application code and authored study material separate, lets the app read a single organized content source, and treats the existing `index.html` as UI/UX reference rather than the product runtime.
